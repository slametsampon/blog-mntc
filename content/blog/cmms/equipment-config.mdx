...
- [Plant Equipment Module `MPE`](#plant-equipment-module-mpe)
  - [Requirement](#requirement)
  - [`Equipment Class Relationship Diagram`](#equipment-class-relationship-diagram)
...
- Trace equipment used spare part
- Perform spare part planning for S/D activities, annual budget based on equipment priority grade (`RBM`).
...
`PPdM` : Preventive Predictive Maintenance including calibration, certification.
`MTBF` : Mean Time Between Failures
`MTBR` : Mean Time Between Repairs.
`FMCA` : Failure Mode and Critical Analysis

`PPdM Template` is a PPdM pre-configuration based on equipment's name (motor, pump, DCS, PLC, compressor etc.) which lists the related work on that equipment.
...
- Equipment can be classified (sorted) based on the equipment's name (motor, compressor, pump, control valve, block valve etc.), as well as grouped based on the operating unit (Reformer, CO2 Removal, Reactor, PSA etc.) where the equipment is installed.
- Each equipment's name has a spare part template-tree (hierarchy), where the template is registered by the user.
- Users can see detailed specifications of the desired equipment.
- Able to add a list of the spare parts used to the equipment, the number as well as the list of spare parts can be updated if there are changes.
...
![Equipment Class Diagram][EquipClassDiagram]

[EquipClassDiagram]: /static/images/classDiagram/EqpDiagram.jpg
