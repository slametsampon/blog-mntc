...
export default function ListLayoutWithAuthors({
  posts,
  title,
  initialDisplayPosts = [],
  pagination,
  ...
  const sortedAuthors = authorKeys.sort((a, b) => authorCounts[b] - authorCounts[a])
          ...
          <div className="hidden max-h-screen h-full sm:flex flex-wrap bg-gray-50 dark:bg-gray-900/70 shadow-md pt-5 dark:shadow-gray-800/40 rounded min-w-[280px] max-w-[280px] overflow-auto">
            <div className="py-4 px-6">
              {pathname.startsWith('/blog') ? (
                <h3 className="text-primary-500 font-bold uppercase">All Authors</h3>
              ) : (
                <Link
                  href={`/blog`}
                ...
                {sortedAuthors.map((t) => {
                  return (
                    <li key={t} className="my-3">
                      {pathname.split('/author/')[1] === slug(t) ? (
